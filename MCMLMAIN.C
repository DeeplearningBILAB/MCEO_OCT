...
**	propogation in multi-layered turbid media with 
**	embedded object for Class I and Class II signal  
**	of optical coherence tomography in ANSI Standard C.
*
****/
...
void InitOutputData(InputStruct, OutStruct *, PhotonStruct);
...
****/
void PredictDoneTime(double P1, double Pt)	
{
...
****/
void DoOneRun(short NumRuns, InputStruct *In_Ptr, PhotonStruct Photon_Ptr)
{
	register double i_photon;	
	/* index to photon. register for speed.*/
	OutStruct out_parm;		/* distribution of photons.*/
	PhotonStruct photon;
	double num_photons = In_Ptr->num_photons, photon_rep=10000; /** change this number to change the display of the number of photons left for run **/
	double temp = num_photons;
	long i;
#if THINKCPROFILER
	InitProfile(200,200); cecho2file("prof.rpt",0, stdout);
#endif

	InitOutputData(*In_Ptr, &out_parm, Photon_Ptr);
	out_parm.Rsp = Rspecular(In_Ptr->layerspecs);	
	PunchTime(0, "");
	for(i=0; i<In_Ptr->num_scan; i++) { 
		In_Ptr->launchCnt=i;
		...
		do {
			if(temp - i_photon == photon_rep) {
				temp = i_photon;
				printf("%lf photons & %hd runs left, ", i_photon, NumRuns);
				PredictDoneTime(num_photons - i_photon, (num_photons*In_Ptr->num_scan));
				photon_rep *= 10;
			}
			LaunchPhotonObj(out_parm.Rsp, In_Ptr, &photon, i_photon); /** LaunchPhoton is replaced with LaunchPhotonObj by Vijitha et al**/

			do  HopDropSpin(In_Ptr, &photon, &out_parm);
			while (!photon.dead);
		} while(--i_photon);
		printf("%ld launches done.\n", (In_Ptr->launchCnt+1));
		temp = num_photons;
	}
...
****/
int
	main(int argc, char *argv[], PhotonStruct Photon_Ptr) 
{
	...
	ShowVersion("Version 1.0.0, 2016");	/** Updated by Vijitha et al **/
	...
	while(num_runs--)  {
		...
		DoOneRun(num_runs, &in_parm, Photon_Ptr);
	}  
